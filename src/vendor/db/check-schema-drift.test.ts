...
// Project root is three levels up from src/vendor/db/
...
describe("schema-only change without migration", () => {
  it("returns ok:false with reason 'drift' and lists pending SQL", async () => {
    // Build a temp db root that mirrors the real one …
    const tempRoot = makeTempDbRoot();
    cpSync(join(realDbRoot, "migrations"), join(tempRoot, "migrations"), {
      recursive: true,
    });
    // Copy only the schema package so drizzle-kit can find and parse it.
    cpSync(join(realDbRoot, "src/vendor/db"), join(tempRoot, "src/vendor/db"), {
      recursive: true,
    });
    ...
    const schemaFile = join(tempRoot, "src/vendor/db/schema/seo-leads.ts");
...
describe("generator failure", () => {
  it("returns ok:false with reason 'generator_error' when schema cannot be parsed", async () => {
    ...
    const srcDir = join(tempRoot, "src/vendor/db/schema");
...
const cliSrc = join(realDbRoot, "src/vendor/db/check-schema-drift.ts");
