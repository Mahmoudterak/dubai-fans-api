 ...
 *
 * @param dbRoot  Absolute path to the `@workspace/db` package root.
...
export async function checkSchemaDrift(dbRoot: string): Promise<DriftResult> {
  ...
  try {
    ...
    const schemaPath = join(dbRoot, "src/schema/index.ts");
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  // at a synthetic temp directory instead of the real package root).
  ...
  checkSchemaDrift(dbRoot).then((result) => {
    ...
    if (result.reason === "generator_error") {
      ...
      console.error(
        "Fix the error above, then re-run: pnpm --filter @workspace/db run check-drift\n",
      );
    } else {
      console.error(
        "\n❌  Schema drift detected!\n\n" +
          "One or more schema files were changed without a matching migration.\n" +
          "Fix: run `pnpm --filter @workspace/db run generate` and commit the result.\n",
      );
